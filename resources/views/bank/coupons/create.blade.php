@extends('layouts.app', ['wide_layout' => false])

@section('title', __('bank.create_coupon'))

@section('content')
    {!! Form::open(['route' => ['bank.coupons.store']]) !!}
        <div class="card shadow-sm mb-4">
            <div class="card-header">@lang('app.general')</div>
            <div class="card-body">
                <div class="form-row">
                    <div class="col-md">
                        {{ Form::bsText('name', null, [ 'required', 'autofocus' ], __('app.name')) }}
                    </div>
                    <div class="col-md">
                        <label for="icon">@lang('app.icon')</label>
                        <div class="input-group mb-3">
                            {{ Form::text('icon', null, [ 'class' => 'form-control', 'id' => 'icon', 'list' => 'icon-list', 'aria-describedby' => 'icon-addon' ]) }}
                            <div class="input-group-append">
                                <span class="input-group-text" id="icon-addon"></span>
                            </div>
                        </div>
                        <datalist id="icon-list">
                            @foreach(list_fa_icons() as $icon)
                                <option>{{ $icon }}</option>
                            @endforeach
                        </datalist>
                    </div>
                </div>
                <div class="form-row">
                    <div class="col-md">
                        {{ Form::bsNumber('daily_amount', null, [ 'required' ], __('people.daily_amount')) }}
                    </div>
                    <div class="col-md">
                        {{ Form::bsNumber('retention_period', null, [ ], __('people.retention_period'), __('app.in_days')) }}
                    </div>
                </div>
            </div>
        </div>
        <div class="card shadow-sm mb-4">
            <div class="card-header">@lang('app.conditions')</div>
            <div class="card-body">
                <div class="form-row">
                    <div class="col-md">
                        {{ Form::bsNumber('newly_registered_block_days', null, [ ], __('people.block_for_newly_registered'), __('app.days')) }}
                    </div>
                    <div class="col-md">
                        {{ Form::bsNumber('min_age', null, [ ], __('people.min_age'), __('app.in_years')) }}
                    </div>
                    <div class="col-md">
                        {{ Form::bsNumber('max_age', null, [ ], __('people.max_age'), __('app.in_years')) }}
                    </div>
                    <div class="col-md">
                        {{ Form::bsNumber('daily_spending_limit', null, [ ], __('people.daily_spending_limit'), __('people.per_day')) }}
                    </div>
                </div>
            </div>
        </div>
        <div class="card shadow-sm mb-4">
            <div class="card-header">@lang('people.qr_code')</div>
            <div class="card-body">
                <div class="form-row">
                    <div class="col-md">
                        {{ Form::bsCheckbox('qr_code_enabled', 1, null, __('people.with_qr_code')) }}
                    </div>
                    <div class="col-md">
                        {{ Form::bsNumber('code_expiry_days', null, [ ], __('people.code_expiry'), __('app.days')) }}
                    </div>
                </div>
            </div>
        </div>
        <div class="card shadow-sm mb-4">
            <div class="card-header">@lang('app.options')</div>
            <div class="card-body">
                <div class="form-row">
                    <div class="col-md">
                        {{ Form::bsCheckbox('enabled', 1, null, __('app.enabled')) }}
                        {{ Form::bsCheckbox('returnable', 1, null, __('people.returnable')) }}
                    </div>
                    <div class="col-md">
                        {{ Form::bsNumber('order', $default_order, [ 'min' => 0 ], __('app.order')) }}
                    </div>
                </div>
            </div>
        </div>
        <p>
            <x-form.bs-submit-button :label="__('app.create')"/>
        </p>
    {!! Form::close() !!}
@endsection

@push('footer')
    <script>
        $(function () {
            $('#icon').on('change', function() {
                $('#icon-addon').html('<i class="fa fa-' + $(this).val() + '"></i>');
            });
        });
    </script>
@endpush

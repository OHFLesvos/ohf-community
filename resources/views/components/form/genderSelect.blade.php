...
<div class="custom-control custom-radio custom-control-inline">
    ...
    <label class="custom-control-label" for="{{ form_id_string($name, 'm') }}">@icon(male) @lang('app.male')</label>
</div>
<div class="custom-control custom-radio custom-control-inline">
    {{ Form::radio($name, 'f', null, [ 'class' => 'custom-control-input', 'id' => form_id_string($name, 'f') ]) }}
    <label class="custom-control-label" for="{{ form_id_string($name, 'f') }}">@icon(female) @lang('app.female')</label>
</div>
@if ($errors->has($name))

...
<div class="custom-control custom-radio custom-control-inline">
    ...
    <label class="custom-control-label" for="{{ form_id_string($name, 'm') }}"><x-icon icon="person"/> {{ __('Male') }}</label>
</div>
<div class="custom-control custom-radio custom-control-inline">
    {{ Form::radio($name, 'f', null, [ 'class' => 'custom-control-input', 'id' => form_id_string($name, 'f') ]) }}
    <label class="custom-control-label" for="{{ form_id_string($name, 'f') }}"><x-icon icon="person-dress"/> {{ __('Female') }}</label>
</div>
@if ($errors->has($name))

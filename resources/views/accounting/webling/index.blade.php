...
@section('content')
    @unless($periods->isEmpty())
        <p>@lang('Please choose a month with unbooked transactions in an open booking period:')</p>
        @foreach($periods as $period_id => $period)
                    ...
                    @foreach($period->months as $month)
                        <a href="{{ route('accounting.webling.prepare', [ $wallet, 'period' => $period_id, 'from' => $month->date->toDateString(), 'to' => (clone $month->date->endOfMonth())->toDateString() ]) }}" class="list-group-item list-group-item-action">
                            <div class="row">
                                ...
                                <div class="col-auto"><small>{{ $month->transactions }} @lang('Transactions')</small></div>
                            </div>
                        </a>
                    @endforeach
                </div>
            @else
                <x-alert type="info">
                    @lang('No months with unbooked transactions found.')
                </x-alert>
            @endunless
        @endforeach
    @else
        <x-alert type="info">
            @lang('No matching open booking periods found.')
        </x-alert>
    @endunless
@endsection
